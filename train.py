    ...
    for epoch in range(args.start_epoch+1,num_epochs):
        ...
        for phase in ['train','val']:
            ...
            for i, (inputs, labels) in enumerate(dataloders[phase]):
                ...
                # print("============")
                # print(np.array(labels.data.cpu()))
                # print(preds.cpu().numpy())
                running_corrects += torch.sum(preds == labels.data)
                # print(running_corrects)
                # running_corrects += torch.sum(np.array(preds.cpu()) == np.array(labels.data.cpu()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, default="E:\graduate\\resnext\ResNeXt-PyTorch\ImageNet")
    parser.add_argument('--batch-size', type=int, default=8)
    parser.add_argument('--num-class', type=int, default=40)
    ...
    print(f'detaloders {len(dataloders)} ')
    # use gpu or not
    ...
    # Decay LR by a factor of 0.1 every 7 epochs 学习率的调整机制
